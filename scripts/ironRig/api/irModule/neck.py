...
class Neck(Module):
    ...
    def _buildSystems(self):
        ...
        self.__ikSystem = SplineIK(self.shortName+'ik_', ikJoints, self.__numControllers)
        ...
        self.__ikSystem.controllers[-1].name = 'head_ctrl'.format(self.shortName)
    ...
    def __setupNonroll(self):
        nonrollGrp = cmds.createNode('transform', n='{}nonroll_grp'.format(self.shortName))
        nonrollGrp.hide()
        cmds.matchTransform(nonrollGrp, self.__ikSystem.joints[0], pivots=True)

        nonrollJntStart = cmds.duplicate(self.__ikSystem.joints[0], n='{}nonroll_start'.format(self.shortName), po=True)[0]
        nonrollJntEnd = cmds.duplicate(self.__ikSystem.joints[-1], n='{}nonroll_end'.format(self.shortName), po=True)[0]
        nonrollJntStart | nonrollJntEnd

        nonrollIkh = cmds.ikHandle(startJoint=nonrollJntStart, endEffector=nonrollJntEnd, solver='ikRPsolver', n='{}nonroll_ikh'.format(self.shortName))[0]
